...
{ date:"28 Feb", name:"End of S3 courses", desc:"Last day of semester 3", tag:"tag-event", label:"Academic" }
]},
...
{ month:"May – June 2026", events:[
{ date:"15–25 May", name:"Exams S4", desc:"Semester 4 final exams", tag:"tag-exam", label:"Exam" },
{ date:"1–15 Jun", name:"Resit Session S4", desc:"Rattrapage semester 4", tag:"tag-session", label:"Resit" },
{ date:"30 Jun", name:"Summer Vacation", desc:"End of academic year", tag:"tag-holiday", label:"Holiday" }
]}
]
};
...
return DATA.semesters[key].modules.filter(m => {
const a = moduleAvg(m); return a !== null && a >= 10;
}).length;
}

function newModules(key) {
return DATA.semesters[key].modules.filter(m => m.isNew);
}
...
const avg = semAvg(sem), passed = semPassed(sem);
const total = DATA.semesters[sem].modules.length;
...
const decEl = $('m-dec');
if (decEl) {
decEl.textContent = `Decision: ${dec}`;
decEl.className = `mm-d ${dec.includes('Admis') ? 'pass' : 'fail'}`;
}
...
<button class="st ${k === DATA.activeSem ? 'on' : ''}" onclick="switchSem('${k}')">${k}</button>`).join('');
}

function switchSem(key) {
DATA.activeSem = key;
renderSemTabs();
renderMarks(key);
}
...
const eg = $('prof-events');
if (eg) eg.innerHTML = DATA.calendar.flatMap(m => m.events).slice(0, 3).map(e => `
<div class="ev-card">
<div class="ev-date">${e.date}</div>
<div class="ev-nm">${e.name}</div>
<div class="ev-rng">${e.desc}</div>
</div>`).join('');
}
...
newModules(key).forEach(m => list.push({
icon:"📝", title:"New note available!",
sub:`${m.name} · ${key}`, time:"just now", mod: m.name
}));
});
...
<div style="flex:1">
<div style="font-size:13px;font-weight:700">New note available!</div>
<div style="font-size:12px;color:var(--muted)">${modName} · just now</div>
</div>
<button onclick="goSec('marks');flashRow('${modName}');this.parentElement.remove()"
style="padding:6px 12px;background:var(--blue);color:#fff;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;font-family:'Outfit',sans-serif;white-space:nowrap">
See now
</button>`;
...
document.querySelectorAll('.tab-btn').forEach(x => x.className = 'tab-btn off');
this.className = 'tab-btn on';
})
);

/* ══ INIT ══ */
function init() {
const hasNew = Object.keys(DATA.semesters).some(k => newModules(k).length > 0);
$('notif-dot').style.display = hasNew ? 'block' : 'none';
$('bnav-dot').style.display = hasNew ? 'block' : 'none';
renderNotifs('nd-list');
renderNotifs('nd-list-mobile');
renderDashboard();
renderSemTabs();
